...
class Solution{
    ...
    int get(Node *root, bool &check){
        if(root == NULL)return 0;
        if(root->left == root->right)return root->data;
        int l=get(root->left, check);
        int r=get(root->right, check);
        if(l+r != root->data)check = false;
        return root->data;
    }
    
    int isSumProperty(Node *root){
        bool check=true;
        if(root == NULL || root->left == root->right)return check;
        int l=get(root->left, check);
        int r=get(root->right, check);
        if(l+r != root->data)check = false;
        return check;
